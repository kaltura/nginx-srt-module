...
typedef struct {
    ...
    ngx_srt_variable_value_t  *default_value;
    ngx_conf_t                *cf;
    unsigned                   hostnames:1;
    unsigned                   no_cacheable:1;
} ngx_srt_map_conf_ctx_t;
...
static int ngx_libc_cdecl ngx_srt_map_cmp_dns_wildcards(const void *one,
    const void *two);
static void *ngx_srt_map_create_conf(ngx_conf_t *cf);
static char *ngx_srt_map_block(ngx_conf_t *cf, ngx_command_t *cmd,
    void *conf);
static char *ngx_srt_map(ngx_conf_t *cf, ngx_command_t *dummy, void *conf);


static ngx_command_t  ngx_srt_map_commands[] = {

    { ngx_string("map"),
      NGX_SRT_MAIN_CONF|NGX_CONF_BLOCK|NGX_CONF_TAKE2,
      ngx_srt_map_block,
    ...
    { ngx_string("map_hash_max_size"),
      ...
      NGX_SRT_MAIN_CONF_OFFSET,
      offsetof(ngx_srt_map_conf_t, hash_max_size),
      NULL },
    ...
    { ngx_string("map_hash_bucket_size"),
      ...
      ngx_conf_set_num_slot,
      NGX_SRT_MAIN_CONF_OFFSET,
      offsetof(ngx_srt_map_conf_t, hash_bucket_size),
      NULL },
...
static ngx_srt_module_t  ngx_srt_map_module_ctx = {
    NULL,                                  /* preconfiguration */
    NULL,                                  /* postconfiguration */

    ngx_srt_map_create_conf,               /* create main configuration */
...
ngx_module_t  ngx_srt_map_module = {
    NGX_MODULE_V1,
    &ngx_srt_map_module_ctx,               /* module context */
    ngx_srt_map_commands,                  /* module directives */
...
static ngx_int_t
ngx_srt_map_variable(ngx_srt_session_t *s, ngx_srt_variable_value_t *v,
    uintptr_t data)
{
    ngx_srt_map_ctx_t  *map = (ngx_srt_map_ctx_t *) data;
    ...
    value = ngx_srt_map_find(s, &map->map, &val);
    ...
    if (!value->valid) {
        cv = (ngx_srt_complex_value_t *) value->data;

        if (ngx_srt_complex_value(s, cv, &str) != NGX_OK) {
            return NGX_ERROR;
        }
...
static void *
ngx_srt_map_create_conf(ngx_conf_t *cf)
{
    ngx_srt_map_conf_t  *mcf;

    mcf = ngx_palloc(cf->pool, sizeof(ngx_srt_map_conf_t));
    if (mcf == NULL) {
        return NULL;
    }
...
static char *
ngx_srt_map_block(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ngx_srt_map_conf_t  *mcf = conf;
    ...
    ngx_srt_map_ctx_t                *map;
    ngx_srt_variable_t               *var;
    ngx_srt_map_conf_ctx_t            ctx;
    ...
    map = ngx_pcalloc(cf->pool, sizeof(ngx_srt_map_ctx_t));
    if (map == NULL) {
    ...
    var->get_handler = ngx_srt_map_variable;
    ...
    *cf = save;
    ...
    if (ctx.no_cacheable) {
        var->flags |= NGX_SRT_VAR_NOCACHEABLE;
    }

    map->default_value = ctx.default_value ? ctx.default_value:
                                             &ngx_srt_variable_null_value;
    ...
    if (ctx.keys.dns_wc_head.nelts) {
        ...
        ngx_qsort(ctx.keys.dns_wc_head.elts,
                  ...
                  sizeof(ngx_hash_key_t), ngx_srt_map_cmp_dns_wildcards);
...
ngx_srt_map_cmp_dns_wildcards(const void *one, const void *two)
{
...
static char *
ngx_srt_map(ngx_conf_t *cf, ngx_command_t *dummy, void *conf)
{
    u_char                           *data;
    size_t                            len;
    ngx_int_t                         rv;
    ...
    ngx_uint_t                        i, key;
    ngx_srt_map_conf_ctx_t           *ctx;
    ngx_srt_complex_value_t           cv, *cvp;
    ngx_srt_variable_value_t         *var, **vp;
    ngx_srt_compile_complex_value_t   ccv;
    ...
    if (vp) {
        for (i = 0; i < ctx->values_hash[key].nelts; i++) {
            ...
            if (vp[i]->valid) {
                ...
                cvp = (ngx_srt_complex_value_t *) vp[i]->data;
                data = cvp->value.data;
                len = cvp->value.len;
            }
    ...
    var = ngx_palloc(ctx->keys.pool, sizeof(ngx_srt_variable_value_t));
    if (var == NULL) {
        return NGX_CONF_ERROR;
    }
    ...
    if (cv.lengths != NULL) {
        cvp = ngx_palloc(ctx->keys.pool, sizeof(ngx_srt_complex_value_t));
        if (cvp == NULL) {
            return NGX_CONF_ERROR;
        }
...
found:
    ...
    if (value[0].len && value[0].data[0] == '~') {
        ...
        regex->regex = ngx_srt_regex_compile(ctx->cf, &rc);
        if (regex->regex == NULL) {
            return NGX_CONF_ERROR;
        }
